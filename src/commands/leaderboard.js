...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const userDatabasePath = getDatabasePath(guildId, 'userDatabase.json');
            const configPath = getDatabasePath(guildId, 'config.json');
            ...
            const userDatabase = readJsonFile(userDatabasePath);
            if (!userDatabase) {
                return interaction.editReply({
                    content: ':x: Error loading user database.',
                });
            }

            const currentMembers = await interaction.guild.members.fetch();
            let userArray = Object.entries(userDatabase).filter(([userId, userData]) =>
